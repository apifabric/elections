...
resourcename="elections"  # lower case, only
resourcegroup="elections_rg"
...
# git clone https://github.com/apilogicserver/elections.git
# cd elections
...
az group create --name elections_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group elections_rg --sku S1 --is-linux
az container create --resource-group elections_rg --name elections --image apilogicserver/elections:latest --dns-name-label elections --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//elections.westus.azurecontainer.io:5656'
